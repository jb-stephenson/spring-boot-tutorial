...
<c:set var="paramSeparator" value="${fn:contains(url, '?') ? '&' : '?'}" />
...
<c:if test="${page.totalPages != 1}">
	<div class="pagination">
		...
		<c:if test="${block != 0}">
			<a href="${url}${paramSeparator}b=${block - 1}&p=${(block - 1) * size + 1}">&lt;&lt;</a>
		</c:if>
		
		<c:forEach var="pageNumber" begin="${startPage}" end="${endPage}">
			<c:choose>
				<c:when test="${page.number != pageNumber - 1}">
					<a href="${url}${paramSeparator}p=${pageNumber}&b=${block}"><c:out value="${pageNumber}" /></a>
				</c:when>
